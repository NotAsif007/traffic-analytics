...
// Authentic Indian CCTV Stream Sources & Scenarios
const CAMERA_FEEDS: Record<string, { image: string; roadType: string; activeClasses: string[] }> = {
  'CAM-MG-W': {
    image: 'https://images.unsplash.com/photo-1596176530529-78163a4f7af2?w=800&auto=format&fit=crop&q=80', // Indian street traffic with autos & bikes
    roadType: 'Urban Arterial (MG Road)',
    activeClasses: ['AUTO-RICKSHAW 97%', 'MOTORCYCLE 98%', 'CAR 95%'],
  },
  'CAM-MG-C1': {
    image: 'https://images.unsplash.com/photo-1570125909232-eb263c188f7e?w=800&auto=format&fit=crop&q=80', // Indian commercial bus & vehicles
    roadType: 'Major Intersection (Brigade Rd)',
    activeClasses: ['BMTC BUS 99%', 'CAR 96%', 'MOTORCYCLE 94%'],
  },
  'CAM-MG-C2': {
    image: 'https://images.unsplash.com/photo-1588714477688-cf28a50e94f7?w=800&auto=format&fit=crop&q=80', // Indian city road with dense traffic
    roadType: 'Metro Corridor (MG East)',
    activeClasses: ['AUTO-RICKSHAW 95%', 'CAR 97%', 'TEMPO 93%'],
  },
  'CAM-RING-W': {
    image: 'https://images.unsplash.com/photo-1545459720-aac8509eb02c?w=800&auto=format&fit=crop&q=80', // High angle highway flyover
    roadType: 'Elevated Corridor (Hebbal Flyover)',
    activeClasses: ['SUV 98%', 'COMMERCIAL TRUCK 96%', 'CAR 97%'],
  },
  'CAM-IND-N': {
    image: 'https://images.unsplash.com/photo-1568605117036-5fe5e7bab0b7?w=800&auto=format&fit=crop&q=80', // High density Indian mixed junction
    roadType: 'Choke Point (Silk Board Jct)',
...
const DEFAULT_FEED = {
  ...
  roadType: 'City Surveillance Network',
...
export const CCTVStreamPlayer: React.FC<CCTVStreamPlayerProps> = ({
  ...
  const feedKey = Object.keys(CAMERA_FEEDS).find((k) => cameraName.toUpperCase().includes(k)) || 'default';
  ...
  return (
    <div className="relative rounded overflow-hidden border border-[#292932] aspect-video bg-[#050508] group select-none shadow-lg">
      ...
      <div className="absolute inset-0 pointer-events-none p-3">
        {/* Detection Box 1: Auto-Rickshaw / Vehicle */}
        <div className="absolute top-[28%] left-[18%] w-[32%] h-[42%] border-2 border-emerald-400/80 rounded-sm bg-emerald-500/10 transition-all duration-700">
          <div className="absolute -top-5 left-0 px-1 py-0.2 bg-emerald-500 text-[#0d0096] font-mono font-bold text-[9px] uppercase tracking-wider flex items-center gap-1 shadow">
            <span>{feed.activeClasses[0] || 'AUTO-RICKSHAW 96%'}</span>
          </div>
        ...
        <div className="absolute top-[38%] right-[16%] w-[24%] h-[38%] border border-[#38bdf8]/80 rounded-sm bg-[#38bdf8]/10 transition-all duration-700">
