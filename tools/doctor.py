...
async def check_database() -> dict[str, Any]:
    ...
    try:
        ...
        print(
            f"    {CYAN}Tip: Run `docker compose up db -d` or use .env.local to connect to PostgreSQL.{RESET}"
        )
...
def check_ai_subsystems() -> dict[str, Any]:
    ...
    print(
        f"  * ANPR Normalizer & Levenshtein Matcher: {GREEN}PASS (Sim: {res.similarity_score:.2f}){RESET}"
    )
    ...
    print(
        f"  * Synthetic Benchmark Generator: {GREEN}PASS ({len(dataset.vehicles)} vehicles, {len(dataset.all_observations)} sightings){RESET}"
    )
...
def check_frontend_readiness() -> dict[str, Any]:
    ...
    if os.path.exists(dist_dir):
        ...
        print(
            f"  * Production Build (dist/): {YELLOW}NOT BUILT (Run `cd frontend && npm run build`){RESET}"
        )
...
def print_summary(results: list):
    ...
    if all_ok:
        print(
            f"  {BOLD}{GREEN}[OK] SYSTEM OPERATIONAL - ALL SUBSYSTEMS HEALTHY & CONFIGURED{RESET}"
        )
    else:
        print(
            f"  {BOLD}{YELLOW}[WARN] SYSTEM OPERATIONAL WITH LOCAL IN-MEMORY/OFFLINE FALLBACKS{RESET}"
        )
    print("  -------------------------------------------------------------\n")
