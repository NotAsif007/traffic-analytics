...
class VehicleObservationCreate(AppBaseModel):
    ...
    frame_number: int | None = Field(None, ge=0, description="Frame number within video stream")
    ...
    detection_confidence: float | None = Field(
        None, ge=0.0, le=1.0, examples=[0.94], description="Vehicle detector confidence (0.0–1.0)"
    )
    ...
    plate_region: str | None = Field(None, max_length=16, examples=["KA", "IN"])
    ...
    crop_path: str | None = Field(
        None, max_length=512, description="Object-storage path to the cropped vehicle image"
    )
    plate_crop_path: str | None = Field(
        None, max_length=512, description="Object-storage path to the cropped plate image"
    )

    # ---- Embedding reference ----
    embedding_id: str | None = Field(None, max_length=128)
    embedding_model: str | None = Field(None, max_length=64, examples=["resnet50-reid-v2"])

    # ---- Kinematics ----
    estimated_speed_kmh: float | None = Field(None, ge=0.0, le=500.0)
    direction: str | None = Field(None, max_length=32, examples=["N", "270"])
    lane: int | None = Field(None, ge=1, le=20)
    ...
    @classmethod
    def validate_vehicle_class(cls, v: str | None) -> str | None:
...
class VehicleObservationStatusUpdate(AppBaseModel):
    ...
    rejection_reason: str | None = Field(None, description="Required when status is 'rejected'")
...
class ObservationFilters(AppBaseModel):
    ...
    plate_text: str | None = Field(
        None, max_length=20, description="Partial plate text match (case-insensitive)"
    )
    vehicle_class: str | None = None
    min_detection_confidence: float | None = Field(None, ge=0.0, le=1.0)
    min_plate_confidence: float | None = Field(None, ge=0.0, le=1.0)
