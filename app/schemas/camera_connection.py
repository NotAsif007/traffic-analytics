...
class CameraConnectionBase(AppBaseModel):
    ...
    min_travel_time_s: int = Field(
        ..., gt=0, examples=[30], description="Minimum plausible travel time (seconds)"
    )
    max_travel_time_s: int = Field(
        ..., gt=0, examples=[120], description="Maximum plausible travel time (seconds)"
    )
    avg_travel_time_s: int | None = Field(None, gt=0, examples=[60])
    distance_m: float | None = Field(
        None, gt=0, examples=[850.0], description="Route distance in metres"
    )
    connection_type: str | None = Field(None, examples=["direct"])
    notes: str | None = None
    ...
    def validate_connection(self) -> CameraConnectionBase:
        ...
        if self.min_travel_time_s > self.max_travel_time_s:
            raise ValueError("min_travel_time_s must be less than or equal to max_travel_time_s")
        # avg must be within bounds if provided
        if self.avg_travel_time_s is not None and not (
            self.min_travel_time_s <= self.avg_travel_time_s <= self.max_travel_time_s
        ):
            raise ValueError(
...
class CameraConnectionUpdate(AppBaseModel):
    ...
    road_id: uuid.UUID | None = None
    min_travel_time_s: int | None = Field(None, gt=0)
    max_travel_time_s: int | None = Field(None, gt=0)
    avg_travel_time_s: int | None = Field(None, gt=0)
